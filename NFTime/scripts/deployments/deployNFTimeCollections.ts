...
// Types
import { NFTime_Snakes, NFTime_Snakes__factory } from '../../typechain-types';
...
async function deployNftMarketplace(chainId: number) {
  ...
  const animalFactory: NFTime_Snakes__factory = await ethers.getContractFactory(
    'NFTime_Snakes',
    deployer
  );
  log('Deploying Contract...');
  const animal: NFTime_Snakes = await animalFactory.deploy();
