...
contract NFTime is ReentrancyGuard {
    struct Listing {
        uint256 price;
        address seller;
    }
    ...
    modifier notListed(
        ...
        if (listing.price > 0) {
            revert('NFTime: this item is already listed');
        }
        _;
    }
    ...
    modifier isListed(address nftAddress, uint256 tokenId) {
        Listing memory listing = s_listings[nftAddress][tokenId];
        if (listing.price <= 0) {
            revert('NFTime: this item is not listed');
        }
        _;
    }
    ...
    {
        if (price <= 0) {
            revert('NFTime: price must be above zero');
        }
        IERC721 nft = IERC721(nftAddress);
        if (!(nft.isApprovedForAll(msg.sender, address(this)) || nft.getApproved(tokenId) == address(this))) {
            revert("ERC721: NFTime don't have access to this item");
        }
        ...
        nonReentrant
    {
        ...
        if (msg.value < listedItem.price) {
            revert("NFTime: you didn't pay enough ETH");
        }
        s_proceeds[listedItem.seller] += msg.value;

        delete (s_listings[nftAddress][tokenId]);
        // Thay đổi từ safeTransferFrom sang transferFrom
    ...
    function withdrawProceeds() external {
        ...
        if (proceeds <= 0) {
            revert("NFTime: you don't have any earnings");
        }
